...
/// 
/// </summary>
public class SlidingWindowAlgorithmTests:IAsyncLifetime
{
    ...
    {
        var algorithm = new SlidingWindowAlgorithm(_store);
    ...
    public async Task EvaluateAsync_BurstAcrossWindowBoundary_IsStillLimited()
    {
        ...
        var algorithm = new SlidingWindowAlgorithm(_store);
        var context = BuildContext("1.2.3.5", limit: 4, window: window);
    ...
    {
        var window = TimeSpan.FromMilliseconds(3000);
        var algorithm = new SlidingWindowAlgorithm(_store);
        var context = BuildContext("1.2.3.5", limit: 4, window: window);
        ...
        await Task.Delay(window + TimeSpan.FromMilliseconds(2800));
